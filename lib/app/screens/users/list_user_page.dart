import 'package:desarrollo_jmas/app/screens/users/user_area_permissions_page.dart';
import 'package:desarrollo_jmas/app/configs/controllers/areas_controller.dart';
import 'package:desarrollo_jmas/app/configs/controllers/role_controller.dart';
...
import 'package:desarrollo_jmas/app/widgets/forms/customTextFieldAzul.dart';
...
class _ListUserPageState extends State<ListUserPage> {
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: CustomListaDesplegableTipo<Areas>(
                          value: selectedArea,
                          labelText: 'Area',
                          items: _areas,
                          onChanged: (valueArea) {
                            setDialogState(
                              () {
                                selectedArea = valueArea;
                              },
                            );
                          },
                          validator: (valueArea) {
                            if (valueArea == null) {
                              return 'Debe seleccionar un area';
                            }
                            return null;
                          },
                          itemLabelBuilder: (area) =>
  ...
  Future<void> _showEditDialog(Users user) async {
    ...
    Areas? selectedArea = _areas.firstWhere(
      (area) => area.idArea == user.idArea,
      orElse: () => _areas.elementAt(10),
    );
  ...
  Future<void> _showAreaPermissions(Users user) async {
    final result = await Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => UserAreaPermissionsPage(user: user),
      ),
    );

    // Si se guardaron cambios (result == true), recargar los usuarios
    if (result == true) {
      _loadUsers();
    }
  }
