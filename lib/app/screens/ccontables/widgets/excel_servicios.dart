  ...
  static Future<void> generateExcelSalidasServicios({
    ...
    try {
      ...
      for (var salida in salidasServicios) {
        ...
        if (!salidasByServiceJuntaContratista[salida.idProducto]!.containsKey(
          salida.id_Junta,
        )) {
          salidasByServiceJuntaContratista[salida.idProducto]![salida
                  .id_Junta!] =
              {};
        ...
        if (!salidasByServiceJuntaContratista[salida.idProducto]![salida
                .id_Junta]!
            .containsKey(contratistaId)) {
          salidasByServiceJuntaContratista[salida.idProducto]![salida
                  .id_Junta]![contratistaId] =
              [];
          totalCostoByServiceJuntaContratista[salida.idProducto]![salida
                  .id_Junta]![contratistaId] =
              0;
        ...
        for (var juntaId
          ...
          for (var contratistaId
              in totalCostoByServiceJuntaContratista[productId]![juntaId]!
                  .keys) {
            totalCargo +=
                totalCostoByServiceJuntaContratista[productId]![juntaId]![contratistaId] ??
                0;
          }
        }
      }
      ...
      for (var productId in salidasByServiceJuntaContratista.keys) {
        ...
        for (var juntaId in salidasByServiceJuntaContratista[productId]!.keys) {
          ...
          for (var contratistaId
            ...
            if (contratistaId > 0) {
              nombreContratista =
                  contratistasMap[contratistaId] ??
                  'Contratista $contratistaId';
            }

            // Concepto combinado: servicio + junta + contratista
            String conceptoMovimiento =
                '${product.prodDescripcion?.toUpperCase() ?? ''} - ${junta.juntaNombre?.toUpperCase() ?? ''}';
            if (contratistaId > 0) {
              conceptoMovimiento += ' - $nombreContratista';
            }

            // === FILA DE CARGO (cuenta de cargo de la junta) ===
            sheet
                .getRangeByName('A$currentRow')
                .setText(junta.juntaCuentaCargo ?? '0');
            ...
            // === FILA DE ABONO (cuenta de abono de la junta) - MISMO CONCEPTO ===
            sheet
                .getRangeByName('A$currentRow')
                .setText(junta.juntaCuentaAbono ?? '0');
            sheet.getRangeByName('A$currentRow').cellStyle = styleInfoData;
