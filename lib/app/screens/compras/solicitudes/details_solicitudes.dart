import 'package:desarrollo_jmas/app/screens/compras/solicitudes/widgets/dialog/autorizacion_dialog.dart';
import 'package:desarrollo_jmas/app/screens/compras/solicitudes/widgets/dialog/validacion_dialog.dart';
import 'package:desarrollo_jmas/app/screens/compras/solicitudes/widgets/pdf_solicitud_compra.dart';
...
import 'package:desarrollo_jmas/app/configs/controllers/solicitud_autorizaciones_controller.dart';
import 'package:desarrollo_jmas/app/configs/controllers/solicitud_compras_controller.dart';
import 'package:desarrollo_jmas/app/configs/controllers/solicitud_validaciones_controller.dart';
...
class _DetailsSolicitudPageState extends State<DetailsSolicitudPage> {
  ...
  late Future<Map<int, String>> _ccProductosFuture;
  ...
  void initState() {
    ...
    _ccProductosFuture = _loadCCProductos();
  }

  Future<Map<int, String>> _loadCCProductos() async {
    try {
      final ccontables = await _ccontablesController.listCcontables();
      final Map<int, String> ccProductosMap = {};

      for (var cc in ccontables) {
        if (cc.idProducto != null &&
            cc.ccProducto != null &&
            cc.ccProducto!.isNotEmpty) {
          ccProductosMap[cc.idProducto!] = cc.ccProducto!;
        }
      }

      return ccProductosMap;
    } catch (e) {
      throw Exception('Error al cargar ccProductos: $e');
    }
  }
  ...
  Future<bool> _puedeValidar() async {
    final solicitudPrincipal = widget.solicitudes.first;

    return await _tienePermisoValidar() &&
        solicitudPrincipal.scEstado.toLowerCase() == 'tramite' &&
        solicitudPrincipal.idUserValida == null;
  }

  Future<bool> _puedeAutorizar() async {
    final solicitudPrincipal = widget.solicitudes.first;

    return await _tienePermisoAutorizar() &&
        solicitudPrincipal.scEstado.toLowerCase() == 'validada' &&
        solicitudPrincipal.idUserAutoriza == null;
  }

  Future<bool> _tienePermisoValidar() async {
    final solicitudPrincipal = widget.solicitudes.first;
    final permisosController = UserAreaPermisosController();

    // Obtener áreas permitidas para el usuario
    final areasPermitidas =
        await _obtenerAreasPermitidas(int.parse(widget.idUser));

    print('DEBUG - Áreas permitidas: $areasPermitidas');
    print('DEBUG - Área de la solicitud: ${solicitudPrincipal.idArea}');

    // Verificar si la solicitud está en una área permitida
    if (!areasPermitidas.contains(solicitudPrincipal.idArea)) {
      print('DEBUG - El usuario NO tiene acceso a esta área');
      return false;
    }

    // Obtener código del área corregido
    final codigoArea = _obtenerCodigoArea(solicitudPrincipal.idArea);
    print('DEBUG - Código área para permisos: $codigoArea');

    // Verificar si el usuario tiene permiso para validar en esta área
    final tienePermiso = await permisosController.validarPermiso(
        int.parse(widget.idUser), codigoArea, 'validar');

    print('DEBUG - ¿Tiene permiso para validar? $tienePermiso');
    return tienePermiso;
  }

  Future<bool> _tienePermisoAutorizar() async {
    final solicitudPrincipal = widget.solicitudes.first;
    final permisosController = UserAreaPermisosController();
    ...
    // Verificar si el usuario tiene permiso para autorizar en esta área
    final tienePermiso = await permisosController.validarPermiso(
        int.parse(widget.idUser), codigoArea, 'autorizar');

    print('DEBUG - ¿Tiene permiso para autorizar? $tienePermiso');
    return tienePermiso;
  }

  Future<List<int>> _obtenerAreasPermitidas(int idUser) async {
    // Casos especiales
    final casosEspeciales = {
    ...
    // Si el usuario está en los casos especiales, retornar sus áreas
    if (casosEspeciales.containsKey(idUser)) {
      return casosEspeciales[idUser]!;
    }

    // Para usuarios normales, obtener su área específica
    final userController = UsersController();
    final usuario = await userController.getUserById(idUser);

    if (usuario?.idArea != null) {
      return [usuario!.idArea!];
    }

    return []; // Si no tiene área, no mostrar nada
  }
  ...
  Future<void> _descargarPDFSolicitud() async {
    ...
    try {
      ...
      final ccProductosMap = await _ccProductosFuture;

      // Obtener validaciones y autorizaciones
      final validaciones = await _validacionesFuture;
      final autorizaciones = await _autorizacinoesFuture;

      // Obtener fechas de validación y autorización
      final fechaValidacion =
          validaciones.isNotEmpty ? validaciones.first.svFecha : null;
      final fechaAutorizacion =
          autorizaciones.isNotEmpty ? autorizaciones.first.saFecha : null;
      ...
      final productosParaPDF = productosAgrupados.entries.map((entry) {
        ...
        // Obtener la clave (ccProducto) del mapa de ccProductos
        final clave = ccProductosMap[idProducto]?.isNotEmpty == true
            ? ccProductosMap[idProducto]!
            : '';

        final descripcionProducto =
            producto?.prodDescripcion ?? 'Producto desconocido';

        final cantidadTotal = solicitudesProducto.fold(
            0.0, (sum, solicitud) => sum + solicitud.scCantidadProductos);

        final costoTotal = solicitudesProducto.fold(
            0.0, (sum, solicitud) => sum + solicitud.scTotalCostoProductos);

        final costoUnitario =
            cantidadTotal > 0 ? costoTotal / cantidadTotal : 0.0;

        return {
          'idProducto': idProducto,
          'clave': clave, // Nueva clave para la columna "Clave"
  ...
  Color _getAreaColor(int? idArea) {
    final mapaColoresAreas = {
      1: Colors.grey, // Metal - Plateado
      2: Colors.yellow, // Rayo - Amarillo
      3: Colors.green, // Planta - Verde
      4: Colors.cyan, // Agua - Cian/Azul
      5: Colors.red, // Fuego - Rojo
      6: Colors.blue.shade900, // Oscura - Azul Oscuro/Azul rey
      7: Colors.purple, // Psíquica - Morado
      8: Colors.orange, // Lucha - Naranja
      9: Colors.lightGreen, // Dragón - Verde Amarillento
      10: Colors.brown, // Bicho - Verde pardo
      11: Colors.white, // Incolora - Blanco
    };

    return mapaColoresAreas[idArea] ?? Colors.grey.shade100;
  }

  String _obtenerCodigoArea(int? idArea) {
    final mapaAreas = {
      1: 'Metal',
      2: 'Rayo',
      3: 'Planta',
    ...
    return mapaAreas[idArea] ?? 'Otro';
  }

  String _obtenerNombreArea(int? idArea) {
    final mapaAreas = {
      1: 'Coordinación',
      2: 'Comercial / Atención a Usuarios',
      3: 'Comercial / Cobranza',
      4: 'Técnica / Infraestructura Hídrica',
      5: 'Técnica / Operación y Mantenimiento',
      6: 'Técnica / Fuentes de Abastecimiento',
      7: 'Financiera / Contabilidad',
      8: 'Financiera / Sistemas',
      9: 'Financiera / Almacén',
      10: 'Financiera',
      11: 'Otro'
    };

    return mapaAreas[idArea] ?? 'Área desconocida';
  }

  String _obtenerDescripcionArea(int? idArea) {
    final mapaAreas = {
      1: 'Coordinación',
      2: 'Comercial / Atención a Usuarios',
      3: 'Comercial / Cobranza',
  ...
  Widget build(BuildContext context) {
    ...
    final solicitudPrincipal = widget.solicitudes.first;
    final areaColor = _getAreaColor(solicitudPrincipal.idArea);
    final nombreArea = _obtenerNombreArea(solicitudPrincipal.idArea);
    final descripcionArea = _obtenerDescripcionArea(solicitudPrincipal.idArea);
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            ...
            Text(
              'Área: $nombreArea - $descripcionArea',
              style: TextStyle(
                fontSize: 20,
                fontWeight: FontWeight.bold,
                color: areaColor.computeLuminance() > 0.5
                    ? Colors.black
                    : Colors.white,
              ),
            ),
        ...
        foregroundColor:
            areaColor.computeLuminance() > 0.5 ? Colors.black : Colors.white,
      ),
      body: _isLoading
          ? Center(
                        ...
                        Expanded(
                          child: FutureBuilder<bool>(
                              ...
                              builder: (context, snapshot) {
                                ...
                                final puedeValidar = snapshot.data ?? false;
                                return FutureBuilder<
                                    List<SolicitudValidaciones>>(
                                  ...
                                  builder: (context, snapshot) {
                                    if (snapshot.connectionState ==
                                      ...
                                      return _buildValidacionCardCompact(
                                          validacion);
                                    } else {
                        ...
                        Expanded(
                          child: FutureBuilder<bool>(
                              ...
                              builder: (context, snapshot) {
                                ...
                                final puedeAutorizar = snapshot.data ?? false;
                                return FutureBuilder<
                                    List<SolicitudAutorizaciones>>(
                        ...
                        children: [
                          ...
                          FutureBuilder<Map<int, String>>(
                            ...
                            builder: (context, ccSnapshot) {
                              ...
                              final ccProductosMap = ccSnapshot.data ?? {};

                              return FutureBuilder<Map<int, Productos>>(
                                ...
                                builder: (context, snapshot) {
                                  ...
                                  // Agrupar productos por ID - CORREGIDO
                                  for (var solicitud in widget.solicitudes) {
                                    productosAgrupados.update(
                                      solicitud.idProducto,
                                      (value) => [...value, solicitud],
                                  ...
                                  return Column(
                                    children:
                                      ...
                                      // Usar ccProducto si está disponible, sino usar la descripción del producto
                                      final nombreProducto = ccProductosMap[
                                                      idProducto]
                                                  ?.isNotEmpty ==
                                              true
                                          ? '${ccProductosMap[idProducto]!} - ${producto?.prodDescripcion ?? 'Producto desconocido'}'
                                          : '${producto?.id_Producto ?? 0} - ${producto?.prodDescripcion ?? 'Producto desconocido'}';

                                      // CORREGIR: Sumar todas las cantidades de las solicitudes agrupadas
                                      final cantidadTotal =
                                          solicitudesProducto.fold(
                                              0.0,
                                              (sum, solicitud) =>
                                                  sum +
                                                  solicitud
                                      ...
                                      final costoUnitario = cantidadTotal > 0
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow('Validado por:',
                '${validacion.idUserValida} - ${widget.userValida?.user_Name ?? 'N/A'}'),
          ],
        ),
      ),
    );
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            _buildInfoRow('Autorizado por:', '${autorizacion.idUserAutoriza}'),
          ],
        ),
      ),
    );
