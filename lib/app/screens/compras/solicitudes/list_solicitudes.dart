import 'package:desarrollo_jmas/app/screens/compras/solicitudes/details_solicitudes.dart';
...
class _ListSolicitudesPageState extends State<ListSolicitudesPage> {
  ...
  Future<void> _reloadData() async {
    ...
    try {
      ...
      final usuarioActual =
          await userController.getUserById(int.parse(widget.idUser));

      // Obtener las áreas permitidas para el usuario
      final areasPermitidas =
          await _obtenerAreasPermitidas(usuarioActual?.id_User);

      // Filtrar solicitudes por áreas permitidas
      List<SolicitudCompras> solicitudesFiltradas =
          solicitudes.where((solicitud) {
        return areasPermitidas.contains(solicitud.idArea);
      }).toList();

      // Ordenar solicitudes por fecha (más reciente primero)
      solicitudesFiltradas.sort((a, b) => b.scFecha.compareTo(a.scFecha));

      setState(() {
        _allSolicitudes = solicitudesFiltradas;
  ...
  Future<void> _loadData() async {
    try {
      ...
      setState(() {
        _allSolicitudes = solicitudesFiltradas;
        _filteredSolicitudes = solicitudesFiltradas;
  ...
  Future<List<int>> _obtenerAreasPermitidas(int? idUser) async {
    ...
    if (idUser != null && casosEspeciales.containsKey(idUser)) {
      return casosEspeciales[idUser]!;
    }

    // Para usuarios normales, obtener su área específica
    final userController = UsersController();
    final usuario = await userController.getUserById(idUser ?? 0);
  ...
  String _obtenerNombreAreaCompleto(int? idArea) {
    final mapaAreasCompleto = {
      1: 'Coordinación',
      2: 'Comercial / Atención a Usuarios',
      3: 'Comercial / Cobranza',
      4: 'Técnica / Infraestructura Hídrica',
    ...
    return mapaAreasCompleto[idArea] ?? 'Área desconocida';
  }
  ...
  Future<bool> _puedeVerSolicitud(SolicitudCompras solicitud) async {
    final userController = UsersController();
    final usuarioActual =
        await userController.getUserById(int.parse(widget.idUser));
    final areasPermitidas =
    ...
    final puedeVer = areasPermitidas.contains(solicitud.idArea);

    if (!puedeVer) {
      final nombreAreaSolicitud = _obtenerNombreAreaCompleto(solicitud.idArea);
      final areasUsuario =
          areasPermitidas.map(_obtenerNombreAreaCompleto).join(', ');

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
              'No tienes permisos para ver solicitudes del área: $nombreAreaSolicitud. '
              'Tus áreas permitidas: $areasUsuario'),
          backgroundColor: Colors.red,
        ),
      );
    }
  ...
  Widget _buildSolicitudCard(
    ...
    return Card(
      ...
      child: Container(
        ...
        child: InkWell(
          onTap: () async {
            // Verificar si el usuario puede ver esta solicitud (misma área)
            final puedeVer = await _puedeVerSolicitud(solicitudPrincipal);
            if (!puedeVer) {
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text(
                      'No tienes permisos para ver solicitudes del área: $nombreArea'),
                  backgroundColor: Colors.red,
                ),
              );
              return;
            ...
            final todasLasSolicitudes = await _solicitudComprasController
                .getSolicitudComprasByFolio(codFolio);
