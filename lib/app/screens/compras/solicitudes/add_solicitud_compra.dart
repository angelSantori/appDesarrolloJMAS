...
import 'package:desarrollo_jmas/app/widgets/buscar_producto.dart';
...
import 'package:desarrollo_jmas/app/widgets/tabla_productos.dart';
...
class _AddSolicitudCompraState extends State<AddSolicitudCompra> {
  ...
  Map<int, String> _cuentasContables = {};
  ...
  Future<void> _buscarCuentaContable(int productoId) async {
    try {
      final cuentas = await _ccontablesController.listCCxProducto(productoId);
      if (cuentas.isNotEmpty && cuentas.first.ccProducto != null) {
        setState(() {
          _cuentasContables[productoId] = cuentas.first.ccProducto!;
        });
      }
    } catch (e) {
      print('Error al buscar cuenta contable para producto $productoId: $e');
    }
  }
  ...
  void _agregarProducto() {
    if (_selectedProducto != null && _cantidadController.text.isNotEmpty) {
      ...
      setState(() {
        ...
        // Buscar la cuenta contable del producto agregado
        _buscarCuentaContable(_selectedProducto!.id_Producto!);
  ...
  SolicitudCompras _crearSolicitudCompra(Map<String, dynamic> producto) {
    return SolicitudCompras(
      ...
      idUserValida: null,
      idUserAutoriza: null,
      idArea: _selectedUser?.idArea ?? 11,
    );
  }
