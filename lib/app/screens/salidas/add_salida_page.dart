import 'dart:convert';
import 'package:desarrollo_jmas/app/screens/salidas/widgets/pdf_salida.dart';
...
import 'package:desarrollo_jmas/app/configs/controllers/presupuestos_controller.dart';
...
import 'package:desarrollo_jmas/app/widgets/buscar_calle_widget.dart';
import 'package:desarrollo_jmas/app/widgets/buscar_colonia_widget.dart';
import 'package:desarrollo_jmas/app/widgets/buscar_padron.dart';
import 'package:desarrollo_jmas/app/widgets/buscar_producto_widget.dart';
...
import 'package:desarrollo_jmas/app/widgets/generales.dart';
import 'package:desarrollo_jmas/app/widgets/mensajes.dart';
import 'package:desarrollo_jmas/app/widgets/tabla_productos_salida.dart';
...
class _AddSalidaPageState extends State<AddSalidaPage> {
      ...
      for (int i = 0; i < _productosAgregados.length; i++) {
        ...
        if (producto['id'] == 40050558 || producto['id'] == 40050557) {
          ...
          if (_selectedJunta != null &&
              !_juntasEspeciales.contains(_selectedJunta!.idJunta)) {
  ...
  Future<void> _buscarPrespuestoByFolio() async {
    ...
    if (folio.isEmpty || folio.length < 4) {
      showAdvertence(
        context,
        'Ingrese un folio de presupuesto válido (ej: PRE1)',
      );
      return;
    }
    ...
    try {
      final presupuestos = await _presupuestosController.getPresupuestoByFolio(
        folio,
      );
      ...
      if (presupuestos == null || presupuestos.isEmpty) {
        showAdvertence(
          context,
          'No se encontró el presupuesto con el folio: $folio',
        );
        setState(() {
      ...
      final presupuestoUtilizado = presupuestos
          .where((p) => !p.presupuestoEstado)
          .toList();
  ...
  void _cargarProductosPresupuestoEnSalida() async {
    ...
    for (var presupuesto in _presupuestosSeleccionados) {
      try {
        final producto = await _productosController.getProductoById(
        ...
        if (producto != null) {
          ...
          final double precioUnitarioPresupuesto =
              ...
              ? presupuesto.presupuestoTotal / presupuesto.presupuestoUnidades
              : 0.0;
              ...
              return TableRow(
                children: [
                  ...
                  FutureBuilder<Productos?>(
                    future: _productosController.getProductoById(
                      presupuesto.idProducto,
                    ),
  ...
  Future<void> _loadDataSalidas() async {
    ...
    List<Contratistas> contratistas = await _contratistasController
        .listContratistas();
  ...
  void _agregarProducto() async {
    if (_selectedProducto != null && _cantidadController.text.isNotEmpty) {
      ...
      setState(() {
        final double precioTotal = _calcularPrecioConDescuento(
          _selectedProducto!,
          cantidad,
        );
      ...
      showAdvertence(
        context,
        'Debe seleccionar un producto y definir la cantidad.',
      );
    }
  }
  ...
  Future<void> _actualizarEstadoPresupuestos() async {
    try {
      ...
      final presupuestosActualizados = _presupuestosSeleccionados.map((
        presupuesto,
      ) {
        return presupuesto.copyWith(presupuestoEstado: false);
      }).toList();
  ...
  Future<void> _guardarSalida() async {
    if (_productosAgregados.isEmpty) {
      showAdvertence(
        context,
        'Debe agregar productos antes de guardar la salida.',
      );
      return;
    }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        salidasGuardadas = await _salidasController.addMultipleSalidas(
          salidasParaGuardar,
        );
        ...
        if (salidasGuardadas == null || salidasGuardadas.isEmpty) {
          ...
          for (int i = 0; i < _productosAgregados.length; i++) {
            ...
            final productoActualizado = await _productosController
            ...
            bool editResult = await _productosController.editProducto(
              productoActualizado,
            );
  ...
  Future<void> _generarPDFConFolio(String folio) async {
    ...
    try {
      await generarPdfSalida(
        ...
        folio: folio,
        presupuestoFolio: !_mostrarPresupuesto
            ? 'N/A'
  ...
  Salidas _crearSalida(Map<String, dynamic> producto) {
    return Salidas(
      id_Salida: 0,
      salida_CodFolio: '',
      salida_PresupuestoFolio: !_mostrarPresupuesto
          ? 'N/A'
          : _folioPresupuestoController.text,
      salida_Estado: true,
                                ...
                                Row(
                                  children: [
                                    ...
                                    if (_mostrarContratista) ...[
                                      Expanded(
                                        child: CustomAutocompleteField<Contratistas>(
  ...
  Future<void> _onOrdenServicioSelected(OrdenServicio? orden) async {
    ...
    if (orden.idUserAsignado != null) {
      final userAsignado = await _usersController.getUserById(
        orden.idUserAsignado!,
      );
      setState(() {
