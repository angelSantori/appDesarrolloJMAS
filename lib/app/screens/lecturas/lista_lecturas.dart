import 'package:desarrollo_jmas/app/screens/lecturas/details_lectura.dart';
import 'package:desarrollo_jmas/app/configs/controllers/lectenviar_controller.dart';
import 'package:desarrollo_jmas/app/configs/controllers/problemas_lectura_controller.dart';
...
class _ListLecturasScreenState extends State<ListLecturasScreen> {
  ...
  Future<void> _downloadTxtFile() async {
    if (_periodoController.text.isEmpty) {
      _showSnackBar('Ingrese un período para descargar');
      return;
    }
    ...
    try {
      ...
      final todasLasLecturas = await _controller.listLectEnviar();
      final lecturasDelPeriodo = todasLasLecturas
          .where((lectura) => lectura.lePeriodo == _periodoController.text)
          .toList();

      if (lecturasDelPeriodo.isEmpty) {
        _showSnackBar(
            'No se encontraron lecturas para el período ${_periodoController.text}');
        return;
      }

      // Construir el contenido del archivo
      final String contenido = _buildTxtContent(lecturasDelPeriodo);

      // Descargar el archivo
      _downloadFileWeb(contenido, 'lecturas_${_periodoController.text}.txt');

      _showSnackBar('Archivo descargado exitosamente');
    } catch (e) {
  ...
  String _buildTxtContent(List<LELista> lecturas) {
    final buffer = StringBuffer();

    for (final lectura in lecturas) {
      // Construir cada línea según el formato de lect2.txt
      // Usando los valores por defecto del modelo C#
      final line = [
        '"Jmas"', // leCampo1
        '"SERGIO"', // leCampo2
        '0', // leCampo3
        ...
        '0', // leCampo8
        '"01/01/1900"', // leCampo9
        '"01/01/1900"', // leCampo10
        '"${lectura.leCuenta ?? ""}"', // leCuenta
        '"${_escapeCsvField(lectura.leNombre ?? "")}"', // leNombre
        '"${_escapeCsvField(lectura.leDireccion ?? "")}"', // leDireccion
        '""', // leCampo11
        lectura.leId?.toString() ?? "", // leId
        '"${lectura.lePeriodo ?? ""}"', // lePeriodo
        '"${lectura.leFecha ?? 'Sin Fecha'}"', // leFecha (formateado como string)
        '"D"', // leCampo12
        '"V"', // leCampo13
        '":"', // leCampo14
        '"${lectura.leNumeroMedidor ?? ""}"', // leNumeroMedidor
        lectura.leLecturaAnterior?.toString() ?? "", // leLecturaAnterior
        lectura.leLecturaActual?.toString() ?? "", // leLecturaActual
        '"1"', // idProblemaLectura (valor por defecto)
        '"${lectura.leRuta ?? ""}"', // leRuta
        '""', // leCampo15
        '""', // leCampo16
        lectura.leCampo17?.toString() ?? "0", // leCampo17
        '"N"', // leCampo18
        '"C1"', // leCampo19
        '0', // leCampo20
        '0', // leCampo21
...
// Función para escapar campos CSV (manejar comillas dentro del texto)
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    // Botón de descarga
                    ElevatedButton(
                      onPressed: _isLoading ? null : _downloadTxtFile,
                      style: ElevatedButton.styleFrom(
