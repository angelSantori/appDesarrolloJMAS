...
class DetailsDescuentoSocial extends StatefulWidget {
  final DescuentosSociales descuentoSocial;

  const DetailsDescuentoSocial({super.key, required this.descuentoSocial});

  @override
  State<DetailsDescuentoSocial> createState() => _DetailsDescuentoSocialState();
}

class _DetailsDescuentoSocialState extends State<DetailsDescuentoSocial> {
  ...
  DescuentoSocialPDF? _pdfExistente;
  ...
  Future<void> _verificarPdfExistente() async {
    try {
      final nombreBuscado = '${widget.descuentoSocial.dsFolio}.pdf';
      final pdfs = await _pdfController.searchDescuentoSocialPdf(
        name: nombreBuscado,
      );
  ...
  Future<void> _descargarPdf() async {
    ...
    try {
      final pdfBytes = await _pdfController.downloadPdf(
        _pdfExistente!.idDescuentoSocualPDF,
      );

      if (pdfBytes != null && pdfBytes.isNotEmpty) {
        // Método para Flutter Web
        await _descargarArchivoWeb(pdfBytes, _pdfExistente!.nombreDocPdfDS);
  ...
  void _mostrarDialogoDescargaExitosa() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'El PDF ${_pdfExistente!.nombreDocPdfDS} se ha descargado exitosamente.',
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Detalles - ${widget.descuentoSocial.dsFolio}'),
              ...
              child: Column(
                children: [
                      ...
                      children: [
                              ...
                              children: [
                                Column(
                                  ...
                                  children: [
                                    ...
                                    _buildInfoItem(
                                      'Folio',
                                      widget.descuentoSocial.dsFolio,
                                    ),
                                    _buildInfoItem(
                                      'Fecha',
                                      _formatearFecha(
                                        widget.descuentoSocial.dsFecha,
                                      ),
                                    ),
                                    _buildInfoItem(
                                      'Razón del Descuento',
                                      widget.descuentoSocial.dsRazonDescuento,
                                    ),
                                  ],
                                ),
                                      ...
                                      children: [
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                'PDF disponible: ${_pdfExistente!.nombreDocPdfDS}',
                                                style: TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  color: Colors.green.shade700,
                                                ),
                                              ...
                                              Text(
                                                'Fecha del PDF: ${_formatearFecha(DateTime.parse(_pdfExistente!.fechaDocPdfDS))}',
                                                style: TextStyle(
                        ...
                        Expanded(
                          child: _buildSeccion('Datos del Beneficiario', [
                            _buildInfoItem(
                              'Nombre',
                              widget.descuentoSocial.dsNombreBeneficiario,
                            ),
                            _buildInfoItem(
                              'Cónyuge',
                              widget.descuentoSocial.dsNombreConyugue,
                            ),
                            _buildInfoItem(
                              'Edad',
                              '${widget.descuentoSocial.dsEdad} años',
                            ),
                            _buildInfoItem(
                              'CURP',
                              widget.descuentoSocial.dsCURP,
                            ),
                            _buildInfoItem(
                              'Teléfono',
                              widget.descuentoSocial.dsTelefono,
                            ),
                          ]),
                        ),
                        const SizedBox(width: 16),
                              ...
                              Expanded(
                                child:
                                    _buildSeccion('Información de Vivienda', [
                                      _buildInfoItem(
                                        'Tipo de Vivienda',
                                        widget.descuentoSocial.dsVivienda,
                                      ),
                                    ]),
                              ),
                              Expanded(
                                child: _buildSeccion('Documentación', [
                                  _buildInfoItem(
                                    'Número de INE',
                                    widget.descuentoSocial.dsNumeroINE,
                                  ),
                                  _buildInfoItem(
                                    'Tipo de Comprobante',
                                    widget.descuentoSocial.dsComprobante ??
                                        'No especificado',
                                  ),
                                ]),
                              ),
                  ...
                  _buildSeccion('Comprobante', [
                    if (widget.descuentoSocial.dsImgDocBase64.isNotEmpty)
                      GestureDetector(
                        onTap: () {
                        ...
                        child: Container(
                          ...
                          child: Column(
                            children: [
                              ...
                              Image.memory(
                                _decodeBase64Image(
                                  widget.descuentoSocial.dsImgDocBase64,
                                ),
                                height: 300,
                                fit: BoxFit.contain,
        ...
        child: Stack(
          children: [
            InteractiveViewer(
              ...
              maxScale: 4.0,
              child: Image.memory(
                _decodeBase64Image(widget.descuentoSocial.dsImgDocBase64),
                fit: BoxFit.contain,
                errorBuilder: (context, error, stackTrace) {
                  return Container(
                    color: Colors.grey.shade200,
