...
import 'package:desarrollo_jmas/app/screens/descuentosSociales/descuentosMovils/details_descuento_social_movil.dart';
...
class ListDescuentoSocialMovil extends StatefulWidget {
  const ListDescuentoSocialMovil({super.key});

  @override
  State<ListDescuentoSocialMovil> createState() =>
      _ListDescuentoSocialMovilState();
}

class _ListDescuentoSocialMovilState extends State<ListDescuentoSocialMovil> {
  final DescuentosSocialesMovilsController _controller =
      DescuentosSocialesMovilsController();
  ...
  List<DescuentosSocialesMovilsList> _descuentosSociales = [];
  List<DescuentosSocialesMovilsList> _descuentosFiltrados = [];
  bool _cargando = true;
  bool _error = false;
  ...
  Future<void> _cargarDescuentosSociales() async {
    try {
      ...
      final descuentos = await _controller.listDSM();

      // Ordenar por fecha descendente (más nuevo primero)
      descuentos.sort((a, b) => b.dsmFecha.compareTo(a.dsmFecha));
  ...
  void _filtrarDescuentos() {
    ...
    setState(() {
      _descuentosFiltrados = _descuentosSociales.where((descuento) {
        ...
        // Filtrar por folio
        if (folio.isNotEmpty) {
          matches = matches && descuento.dsmFolio.toLowerCase().contains(folio);
        }

        // Filtrar por CURP
        if (curp.isNotEmpty) {
          matches = matches && descuento.dsmCURP.toLowerCase().contains(curp);
        }

        // Filtrar por número de INE
        if (ine.isNotEmpty) {
          matches =
              matches && descuento.dsmNumeroINE.toLowerCase().contains(ine);
        }
      ...
      _descuentosFiltrados.sort((a, b) => b.dsmFecha.compareTo(a.dsmFecha));
    });
  }
  ...
  void _limpiarBusqueda() {
    ...
    setState(() {
      _descuentosFiltrados = List.from(_descuentosSociales)
        ..sort((a, b) => b.dsmFecha.compareTo(a.dsmFecha));
    });
  }

  Future<void> _verDetalles(DescuentosSocialesMovilsList descuentoList) async {
    try {
      ...
      final descuentoCompleto = await _controller.getDSMById(
        descuentoList.idDescuentoSocialMovil,
      );
      ...
      if (descuentoCompleto != null) {
        ...
        if (mounted) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => DetailsDescuentoSocialMovil(
                descuentoSocial: descuentoCompleto,
              ),
            ),
          );
        }
  ...
  Future<void> _descargarZip() async {
    ...
    try {
      ...
      if (zipBytes != null && zipBytes.isNotEmpty) {
        ...
        final zipFileName =
            'DescuentosSocialesMovils_${_formatearFechaParaArchivo(_selectedDateRange!.start)}_${_formatearFechaParaArchivo(_selectedDateRange!.end)}_${_formatearHoraParaArchivo(now)}.zip';
  ...
  Future<void> _generarExcel() async {
    ...
    try {
      ...
      final descuentosFiltrados = _descuentosSociales.where((descuento) {
        return descuento.dsmFecha.isAfter(
              _selectedDateRange!.start.subtract(const Duration(days: 1)),
            ) &&
            descuento.dsmFecha.isBefore(
      ...
      for (int i = 0; i < descuentosFiltrados.length; i++) {
        ...
        sheet.getRangeByIndex(i + 2, 1).setText(descuento.dsmFolio);
        sheet
            .getRangeByIndex(i + 2, 2)
            .setText(_formatearFecha(descuento.dsmFecha));
        ...
        sheet
            ...
            .setText(descuento.dsmNombreBeneficiario);
        sheet.getRangeByIndex(i + 2, 5).setText(descuento.dsmNombreConyugue);
        sheet.getRangeByIndex(i + 2, 6).setNumber(descuento.dsmEdad.toDouble());
        sheet.getRangeByIndex(i + 2, 7).setText(descuento.dsmCURP);
        sheet.getRangeByIndex(i + 2, 8).setText(descuento.dsmTelefono);
        sheet.getRangeByIndex(i + 2, 9).setText(descuento.dsmRazonDescuento);
        sheet.getRangeByIndex(i + 2, 10).setText(descuento.dsmVivienda);
        sheet.getRangeByIndex(i + 2, 11).setText(descuento.dsmNumeroINE);
        sheet.getRangeByIndex(i + 2, 12).setText(descuento.dsmComprobante);
        sheet
      ...
      final now = DateTime.now();
      final excelFileName =
          'DescuentosSocialesMovils_${_formatearFechaParaArchivo(_selectedDateRange!.start)}_${_formatearFechaParaArchivo(_selectedDateRange!.end)}_${_formatearHoraParaArchivo(now)}.xlsx';
  ...
  Widget _buildListItem(DescuentosSocialesMovilsList descuento) {
    return Card(
      elevation: 2,
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            Text(
              'Fecha: ${_formatearFecha(descuento.dsmFecha)}',
              style: TextStyle(color: Colors.grey.shade600),
            ),
            const SizedBox(height: 2),
            Text(
              'CURP: ${descuento.dsmCURP}',
              style: TextStyle(color: Colors.grey.shade600),
            ),
            const SizedBox(height: 2),
            Text(
              'Beneficiario: ${descuento.dsmNombreBeneficiario}',
              style: TextStyle(color: Colors.grey.shade600),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Lista de Descuentos Sociales Móvil'),
