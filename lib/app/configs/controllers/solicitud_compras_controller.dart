...
class SolicitudComprasController {
  ...
  Future<List<SolicitudCompras>> listSolicitudCompras() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return jsonData
            .map((listSC) => SolicitudCompras.fromMap(listSC))
            .toList();
      } else {
        print(
  ...
  Future<List<SolicitudCompras>> getSolicitudComprasByFolio(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> jsonData = json.decode(response.body);
        return jsonData.map((sc) => SolicitudCompras.fromMap(sc)).toList();
      } else {
        print(
  ...
  Future<List<SolicitudCompras>?> addMultipleSolicitudCompras(
    List<SolicitudCompras> solicitudCompra,
  ) async {
    try {
...
class SolicitudCompras {
  ...
  factory SolicitudCompras.fromMap(Map<String, dynamic> map) {
    return SolicitudCompras(
      ...
      idUserValida: map['idUserValida'] != null
          ? map['idUserValida'] as int
          : null,
      idUserAutoriza: map['idUserAutoriza'] != null
          ? map['idUserAutoriza'] as int
          : null,
      idArea: map['idArea'] != null ? map['idArea'] as int : null,
    );
  }
