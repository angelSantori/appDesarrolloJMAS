...
class BuscarProductoWidget extends StatefulWidget {
  ...
  final CcontablesController ccontablesController;
  ...
  const BuscarProductoWidget({
    ...
    this.onEnterPressed,
    required this.ccontablesController,
  });
...
class _BuscarProductoWidgetState extends State<BuscarProductoWidget> {
  ...
  Future<void> _buscarCuentaContable(int productoId) async {
    try {
      final cuentas =
          await widget.ccontablesController.listCCxProducto(productoId);

      if (cuentas.isNotEmpty) {
        setState(() {
          _ccProducto = cuentas.first.ccProducto;
        });
      } else {
        setState(() {
          _ccProducto = null;
      ...
      print('Error al buscar cuenta contable: $e');
      setState(() {
        _ccProducto = null;
      });
    }
    ...
    if (id.isNotEmpty) {
      ...
      try {
        ...
        if (producto != null) {
          ...
          // Buscar la cuenta contable del producto
          await _buscarCuentaContable(producto.id_Producto!);
          FocusScope.of(context).requestFocus(_cantidadFocusNode);
        } else {
          widget.onAdvertencia('Producto con ID: $id, no encontrado');
          setState(() => _ccProducto = null);
        }
      } catch (e) {
  ...
  void _seleccionarProducto(Productos producto) {
    ...
    _buscarCuentaContable(producto.id_Producto!);
    setState(() {
      _productosSugeridos = [];
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          // CUENTA CONTABLE
                          if (_ccProducto != null &&
                              _ccProducto!.isNotEmpty) ...[
                            Text(
                              'Cuenta Contable: $_ccProducto',
                              style: TextStyle(
                                fontSize: 16,
